...
import { pushPodcasts } from "@/src/Redux/podcast";
import { APICall, formatToCurrency } from "@/src/Utils";
import { updateAnalytics } from "@/src/Redux/analytics";
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search"
                className="text-xs placeholder:text-xs w-full placeholder:text-[#98A2B3] pl-10 pr-3 py-[10px] rounded-lg border border-gray-300 bg-transparent"
              />
            </div>
          </div>
          <Listbox
          ...
          >
            ...
            <Listbox.Options className="absolute mt-1 w-[205px] bg-[#141414] rounded-lg text-xs font-medium z-20">
              ...
              <div className="p-2">
                <Listbox.Option
                  className={"cursor-pointer"}
                  value={{ id: 6, name: "Custom range" }}
                >
                  {({ active, selected }) => (
                    <Popover className={``}>
                      <Popover.Button
                        ref={setReferenceElement}
                        className={`py-[0.63rem] px-2 rounded-lg hover:bg-[#1D2939] w-full text-left `}
                      >
                ...
                <div className="overflow-x-scroll mt-8 md:h-auto h-96 rounded-lg">
                  <table className="border-collapse table-auto w-full whitespace-nowrap">
                    <thead className="text-white text-left border-b-2 border-[#EAECF0]">
                      <tr>
                        <th className="py-4 pl-6 font-medium text-sm">
                          {totalContent} podcasts
                        </th>
                      ...
                      {podcasts.map((podcast) => {
                        return (
                          <tr
                            onClick={() =>
                              navigate.push(
                                `/analytics/podcast-view/${podcast.id}`
                              )
                            }
                            className="hover:bg-grayTrue cursor-pointer"
                          >
                            ...
                            <td className="py-4 border-b pl-6 text-xs border-t border-[#667085] text-right">
                              <div className="">{podcast.subscriber_count}</div>
                            </td>
                            ...
                            <td className="py-4 border-b px-6 text-xs border-t border-[#667085] text-right">
                              <div className="">{podcast.play_count}</div>
                            </td>
                          </tr>
                        );
                      })}
    ...
    <div id="dashboard">
      <div className="relative">
        <div className="flex gap-3 items-center">
          ...
          <div className="text-sm font-medium text-[#66C6BF]">Overview</div>
        </div>
        <div className="pr-5 mt-5">
          <div className={`font-semibold text-xl pb-2`}>Analytics</div>
          <div>
            <p className="text-sm">All podcasts</p>
          </div>
        </div>
      </div>
