...
import {
  archiveEpisode,
  getArchivePodcastsById,
  getEpisodesArchive,
  getPodcastEpisodes,
  getPodcastsById,
  removeArchiveEpisode,
} from "@/src/app/api/publishers";
import Button from "@/src/Components/button";
import Input from "@/src/Components/Input";
import Modal from "@/src/Components/Modal";
import { useAppDispatch, useAppSelector } from "@/src/Hooks";
import { EpisodeModel } from "@/src/Models/episode";
import { PodcastModel } from "@/src/Models/podcast";
...
import { refreshPodcasts } from "@/src/Redux/podcast";
import { formatTimeW } from "@/src/Utils/audio-player";
...
const PodcastView = ({ params }: { params: { podcastId: string[] } }) => {
  ...
  const [isArchive, setIsArchive] = useState(
    params.podcastId[1] == "archive" ? true : false
  );
  ...
  const handleGetEpisodes = async (page?: number) => {
    try {
      console.log(page);
      const response = await APICall(getPodcastEpisodes, [
        params.podcastId[0],
  ...
  const handleGetPodcasts = async () => {
    try {
      const podcastResponse = await APICall(
        isArchive ? getArchivePodcastsById : getPodcastsById,
        params.podcastId[0]
      );
  ...
  return (
    <div id="dashboard">
      <Modal
        ...
        onClose={(val) => setShowTippingModal(val)}
      >
        ...
        <div className="py-20 text-center px-10">
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              ...
              >
                <path
                  d="M0.999999 17.1445L10 8.89453L1 0.644531"
                  stroke="white"
                  stroke-width="1.5"
                />
              </svg>
          ...
          <div className="bg-white rounded-lg p-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                >
                  <path
                    d="M33.941 0.369141H5.96547C2.67082 0.369141 0 3.04477 0 6.34536V34.3713C0 37.6718 2.67082 40.3475 5.96547 40.3475H33.941C37.2356 40.3475 39.9065 37.6718 39.9065 34.3713V6.34536C39.9065 3.04477 37.2356 0.369141 33.941 0.369141Z"
                    fill="#083F62"
                  />
                  <path
                    d="M15.8075 30.3934C16.0039 30.3934 16.2823 30.3329 16.6341 29.961L16.1334 31.9999C15.9674 32.6751 15.5819 33.2758 15.0375 33.7073C14.4932 34.1389 13.821 34.3767 13.1269 34.3835C12.4327 34.3902 11.7561 34.1653 11.2036 33.7443C10.651 33.3233 10.2541 32.7303 10.0751 32.0583L5.67655 15.4096C5.59724 15.1145 5.57747 14.8065 5.61841 14.5036C5.65937 14.2007 5.76021 13.909 5.91504 13.6457C6.06988 13.3823 6.27558 13.1525 6.52014 12.9698C6.76469 12.787 7.04317 12.655 7.33929 12.5813C7.6354 12.5077 7.94319 12.494 8.24466 12.541C8.54613 12.588 8.83521 12.6948 9.095 12.855C9.35479 13.0153 9.58005 13.2259 9.75761 13.4745C9.93517 13.723 10.0615 14.0046 10.1291 14.3026L13.1679 27.4053H13.2284L13.6082 25.9177C13.8132 26.9858 14.5578 30.348 15.8075 30.3934Z"
                    fill="#25AEA4"
                  />
                  <path
                    d="M28.3672 31.8697L28.3284 32.0146C28.1749 32.6358 27.8354 33.1951 27.3554 33.6176C26.8757 34.0401 26.2782 34.3052 25.6435 34.3778C24.911 34.4505 24.1769 34.2567 23.5754 33.8321C22.9739 33.4072 22.5451 32.7798 22.3672 32.0643L19.2507 19.5238H19.1816L16.9133 28.8038C16.8884 28.9095 16.8374 29.0072 16.765 29.0881C16.6926 29.1689 16.6011 29.2303 16.4989 29.2665C16.4046 29.2987 16.3045 29.3103 16.2053 29.3011C15.6874 29.2383 15.3701 28.5378 14.932 27.3227C14.6622 26.5638 14.3298 25.5648 14.0039 24.3583L16.46 14.7151C16.6192 14.0924 16.9814 13.5408 17.489 13.1477C17.9966 12.7546 18.6207 12.5425 19.2623 12.5449C19.9039 12.5474 20.5263 12.7644 21.0309 13.1614C21.5355 13.5585 21.8933 14.1129 22.0478 14.7367L25.2334 27.6059C25.2507 27.6967 25.2723 27.7875 25.2939 27.8783C25.293 27.8833 25.293 27.8885 25.2939 27.8935C25.378 28.2719 25.473 28.6675 25.5787 29.0697C25.6975 29.5238 25.827 29.9583 25.9564 30.367C25.9556 30.3756 25.9556 30.3843 25.9564 30.3929C26.3277 31.4135 26.8392 32.3194 27.4823 32.3389C27.7025 32.3519 27.9982 32.274 28.3672 31.8697Z"
                    fill="#25AEA4"
                  />
                  <path
                    d="M28.6768 30.6079C28.6367 30.7465 28.5624 30.873 28.461 30.9755C28.3229 31.0877 28.1459 31.1405 27.9689 31.1225C27.9111 31.1149 27.8545 31.0998 27.8006 31.0771H27.7876C27.356 30.8609 27.0732 30.1517 26.7236 29.1614C26.4344 28.3376 26.0761 27.2155 25.707 25.8598L29.8574 8.07819C29.9239 7.78508 30.0478 7.50818 30.2221 7.26361C30.3963 7.01905 30.6178 6.81174 30.8729 6.65382C31.1282 6.49592 31.4122 6.39053 31.7085 6.34385C32.0049 6.29717 32.3075 6.31014 32.5988 6.38197C32.8902 6.45382 33.1643 6.58309 33.4052 6.76225C33.646 6.94143 33.8487 7.16688 34.0015 7.42545C34.1545 7.68402 34.2544 7.97053 34.2955 8.26822C34.3367 8.56593 34.3181 8.86882 34.2408 9.15927L28.748 30.4068L28.6768 30.6079Z"
                    fill="#25AEA4"
                  />
                </svg>
                <div className="text-[#667085]">www.wokpa.app</div>
              </div>
              <div className="text-lg text-black font-medium">Copy</div>
            </div>
          </div>
        </div>
      </Modal>
      <div className="relative">
        <div className="h-[160px]">
          {podcast?.cover_picture_url ? (
            <img
              className="rounded-t-3xl h-[160px] w-full o object-cover"
              src={podcast.cover_picture_url}
              alt=""
            />
          ) : (
            <img
              className="rounded-t-3xl h-[160px] w-full o object-cover"
              src={"/images/gradient.jpeg"}
              alt=""
            />
          )}
        </div>
          ...
          <div className="flex gap-4">
            <div className="">
              <div className="!w-[206px] !h-[206px] relative -top-16">
                {podcast?.picture_url ? (
                  <img
                    className="!w-[206px] !h-[206px] rounded-lg border-2 border-[#BEE7E4] object-cover"
            ...
            <div className="flex-1">
              <div className="flex gap-4 items-center">
                <div className="text-sm font-medium">
                  <Link href="/dashboard">All Podcasts</Link>
                </div>
                <div>
                  <svg
                    width="16"
                ...
                <div className="text-sm font-medium text-[#66C6BF]">
                  {podcast?.title ? (
                    ...
                    <div className="h-2 w-48 bg-[#66C6BF] animate-pulse rounded"></div>
                  )}
                </div>
              </div>
              <div className="flex justify-between flex-col gap-3 mt-5">
                <div>
                  <div className="font-medium font-raleway text-3xl">
                    {podcast?.title ? (
                      ...
                      <div className="h-2 w-96 bg-slate-100 animate-pulse rounded"></div>
                    )}
                  </div>
                </div>
                    ...
                    >
                      <p
                        ...
                        dangerouslySetInnerHTML={{
                          __html: podcast?.description as string,
                        }}
                      ></p>
                    </ShowMoreText>
                  ) : (
                ...
                <div className="flex gap-2 items-center text-[#D0D5DD]">
                  <div className="flex gap-1 items-center">
                    ...
                    >
                      <path
                        d="M9 13.6195L13.635 16.417L12.405 11.1445L16.5 7.59699L11.1075 7.13949L9 2.16699L6.8925 7.13949L1.5 7.59699L5.595 11.1445L4.365 16.417L9 13.6195Z"
                        fill="#D0D5DD"
                      />
                    </svg>
                    <div className="text-sm font-semibold">
                      ...
                      <span className="font-normal ml-1">
                        (
                        {Number(
                          podcast?.rating_count ? podcast.rating_count : "0"
                        )}
                        )
                      </span>
                    </div>
                  ...
                  <div className="text-sm">{podcast?.category_name}</div>
                ...
                <div className="space-y-5">
                  <div className="flex gap-5 items-center">
                    <div className="">
                      <div className="text-xs text-[#0D0D0D] font-semibold bg-white rounded-full py-2 px-4">
                        {podcast?.episode_count ? podcast.episode_count : 0}{" "}
                        Episodes
                      </div>
                    </div>
                    <div>
                      <div className="text-sm font-medium">
                        Published:{" "}
                        {moment(podcast?.created_at).format("DD MMM, YYYY")}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 text-sm">
                      ...
                      >
                        <path
                          d="M6.66675 13.3337H9.33341V2.66699H6.66675V13.3337ZM2.66675 13.3337H5.33341V8.00033H2.66675V13.3337ZM10.6667 6.00033V13.3337H13.3334V6.00033H10.6667Z"
                          fill="#E5F5F4"
                        />
                      </svg>

                      <div>{podcast?.play_count} Plays</div>
                    </div>
                    <div className="flex items-center gap-1 text-sm">
                      <svg
                        width="16"
                      ...
                      <div>{formatTimeW(podcast?.total_duration)}</div>
                    </div>
                    <div className="flex items-center gap-1 text-sm">
                      <div>₦</div>
                      <div>0</div>
                  ...
                  <div className="flex gap-4 flex-wrap">
                    <Link
                      href={`/podcast/edit-podcast/${podcast?.id}`}
                      className="rounded-[40px] px-5 font-medium inline-block bg-transparent text-sm !text-white border !border-[#042946] !py-2"
                    >
                      Edit podcast
                    </Link>
                    <Button
                      onClick={() => setShowTippingModal(true)}
                      className="!from-white !to-white text-sm !text-[#042946] !py-2 font-semibold"
                    >
                      Copy tipping link
                    </Button>
                    <Link
                      href={`/live-player/${podcast?.id}`}
                      className="rounded-[40px] px-5 font-medium inline-block bg-gradient-to-r from-[#083F62] to-[#25AEA4] text-sm !text-white border !border-[#042946] !py-2"
                    >
                      View Live
                    </Link>
                    <Link
                      href={`/podcast/distribution/${podcast?.id}/${
                        isArchive ? "archive" : ""
                      }`}
                      className="rounded-[40px] px-5 font-medium inline-block bg-white text-sm !text-[#042946] !py-2"
                    >
                      Distribution
                    </Link>
                    <Link
                      href={`/podcast/social-sharing/${podcast?.id}/${
                        isArchive ? "archive" : ""
                      }`}
                      className="rounded-[40px] px-5 font-medium inline-block bg-transparent text-sm !text-white border !border-[#042946] !py-2"
                    >
                    ...
                    <Link
                      href={`/podcast/embedded-player/${podcast?.id}/${
                        isArchive ? "archive" : ""
                      }`}
                      className="rounded-[40px] px-5 font-medium inline-block bg-gradient-to-r from-[#083F62] to-[#25AEA4] text-sm !text-white border !border-[#042946] !py-2"
                    >
      ...
      <div className="mt-8">
        <div className="flex justify-end gap-4">
          <Button className=" !from-transparent !to-transparent text-sm !text-white border !border-[#042946] !py-2">
            Record new episode
          </Button>
          <Link
            href="/podcast/create-episode"
            className="rounded-[40px] px-5 inline-block bg-white text-sm !text-[#042946] !py-2 font-semibold"
          >
            Upload new episode
          </Link>
        </div>
          ...
          <Tab.List className="flex border-b gap-2 border-[#98A2B3]">
            <Tab className="outline-none">
              {({ selected }) => (
                <div
                  className={`${
                    selected ? "border-b-2 border-[#0D8478]" : "text-[#98A2B3]"
                  } font-semibold px-4 text-xl pb-2`}
                >
                  All Episodes
                </div>
              )}
