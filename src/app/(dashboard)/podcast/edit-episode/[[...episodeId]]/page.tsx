...
import { APICall, handleRequestAnimationFrame } from "@/src/Utils";
...
import { updatePlaybackData } from "@/src/Redux/player";
import {
  getPodcastsById,
  getPodcastEpisodes,
  getPodcastEpisode,
  ...
  const handleReplaceAudio = async () => {
    try {
      ...
      const response = await APICall(
        uploadEpisodeAudio,
        [episode.podcast_id, episode.id, data],
        true
      );
      setIsLoading(false);
    } catch (error: any) {
      ...
      <div className="pr-5">
        ...
        <div>
          <p className="text-sm">
            ...
            file from your computer. Your progress will be retained, and you
            won't need to modify any of the initially entered information. Tap
            the button below to upload a different file. Prior to replacing the
            audio, we suggest downloading the original version as a backup to
            preserve it.
          </p>
        </div>
        <div className="mt-6 space-y-6">
          <a href={episode?.content_url} download target="_blank">
            <Button>Download episode</Button>
          </a>
          <div>
            <Button className="!from-white !to-white !text-[#063150] font-semibold">
              Replace episode
            </Button>
          </div>
        </div>
              ...
              <div className="w-8/12">
                <div className="dropzone border-2 rounded-lg bg-grayTrue border-[#98A2B3] p-10 text-center border-dashed">
                  <div {...getRootProps({ className: "dropzone" })}>
                    ...
                    {audioFile
                      ? audioFile.name
                      : "Drag and drop your audio file here or click to browse from your device. We support mp3, mp4, m4a, wav, 3gp, aac, amr, flac, ogg, ra, asf file. Maximum file size is 300 MB."}
                  </div>
                  <div className="mt-4">
                    <Button
                      onClick={handleReplaceAudio}
                      className="!from-white !to-white !text-[#063150] font-semibold"
                    >
                      {isLoading ? (
                        <svg
                          className="w-5 h-5"
    ...
    <div className="rounded-lg border py-4 px-6 flex items-center gap-4">
      <button onClick={handlePlay}>
        {playbackData.playing ? (
          ...
          >
            <path
              d="M12.666 20.0001V12.0001M19.3327 20.0001V12.0001M29.3327 16.0001C29.3327 23.3639 23.3631 29.3334 15.9993 29.3334C8.63555 29.3334 2.66602 23.3639 2.66602 16.0001C2.66602 8.63628 8.63555 2.66675 15.9993 2.66675C23.3631 2.66675 29.3327 8.63628 29.3327 16.0001Z"
              stroke="#E5F5F4"
              stroke-width="2"
          ...
          >
            <path
              d="M20.0002 36.6663C29.2049 36.6663 36.6668 29.2044 36.6668 19.9997C36.6668 10.7949 29.2049 3.33301 20.0002 3.33301C10.7954 3.33301 3.3335 10.7949 3.3335 19.9997C3.3335 29.2044 10.7954 36.6663 20.0002 36.6663Z"
              stroke="#F2F4F7"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
      ...
      <div className="space-y-4 flex-1">
        <div className="font-bold">
          ...
          <span className="ml-4 text-xs font-normal">
            {playbackData.playtime}/{playbackData.duration}
          </span>
        </div>
        ...
        <Slider
          ...
          onChange={({ x }) => {
            AudioPlayer.loaded && AudioPlayer.seek(x / 100);
          }}
        />
      </div>
    </div>
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const episodesResponse = params.episodeId[2]
          ? await getArchivedEpisodeById(
              params.episodeId[0],
              params.episodeId[1]
            )
          : await getPodcastEpisode(params.episodeId[0], params.episodeId[1]);
  ...
  return (
    <div id="dashboard">
      <Modal
        size="sm"
        open={showEditModal}
        onClose={(val) => setShowEditModal(val)}
      >
      ...
      <Tab.Group>
          ...
          <Tab className="outline-none">
            {({ selected }) => (
              <div
                className={`${
                  selected
                    ? "border-b-2 border-[#0D8478] text-[#0D8478]"
                    : "text-[#98A2B3]"
                } font-semibold px-4 text-sm pb-2`}
              >
                Episode Info
              </div>
            )}
        ...
        <Tab.Panels>
          <Tab.Panel>
            <div className="mt-8 pb-8">
              <div className="pr-5">
                ...
                <div>
                  <p className="text-sm">
                    Here you can customize the podcasts basic information as
                    well as more advanced details such as the RSS Tags
                  </p>
                </div>
              </div>
              <div className="mt-8">
                        ...
                        <div>
                            ...
                            <div className="flex">
                              <div className="w-8/12">
                                ...
                                <div className="flex items-center gap-4">
                                  <Input
                                    className="!bg-[#202939] !text-[#F2F4F7] !border-none"
                                    value={episode?.content_url}
                                    name="email"
                                    placeholder="www.wokpa.com/emax-podcast"
                                  />
                          ...
                          <div className="space-y-8 mt-6">
                              ...
                              <div className="space-y-4">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-sm">
                                    Public (can be listened to by anyone)
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <Field
                                  ...
                                  />
                                  <span className="text-sm">
                                    Private (can't be listened to by anyone)
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <Field
                                  ...
                                  />
                                  <span className="text-sm">
                                    Limited Access (allows creating Paid
                                    subscription)
                                  </span>
                                </div>
                                <ErrorMessage
                              ...
                              <div className="w-[348px] py-4">
                                <div className="text-center">
                                  ...
                                  <div className="text-xs text-[#EAECF0] px-6 mt-4">
                                    We recommend uploading an artwork of at
                                    least 1400x1400 pixels and maximum 5MB. We
                                    support jpg, png, gif and tiff formats.
                                  </div>
                                  <div className="mt-4 space-x-4">
                                    {episode?.picture_url && (
                                      <button
                                        ...
                                        className="bg-[#F9F5FF] rounded-full py-2 px-12 text-sm font-semibold text-[#042946]"
                                      >
                                        Edit
                                      </button>
                                    )}
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => imagePicker()}
                                      className="bg-[#25AEA4] rounded-full py-2 px-4 text-sm font-semibold text-white"
                                    >
                                      Upload image
                                    </button>
                                  </div>
                            ...
                            <div className="flex gap-5">
                              <div className="md:w-6/12">
                                ...
                                />
                                <div className="text-xs text-[#D0D5DD] mt-1">
                                  Listeners want to know what your podcast is
                                  about before they tune in. Hook them in with a
                                  persuasive description that quickly sums up
                                  what the main concept and structure of your
                                  podcast is.
                                </div>
                                <ErrorMessage
                                  name="description"
                            ...
                            <div className="flex gap-5">
                              <div className="md:w-6/12">
                                ...
                                <div className="text-sm font-medium mt-4">
                                  Enable this option if your recording contains
                                  explicit content. This <br /> information is
                                  also exported to your RSS / iTunes feeds.
                                </div>
                                <div className="flex gap-10">
                                    ...
                                    >
                                      <g>
                                        ...
                                        <path
                                          ...
                                          fill="url(#paint0_linear_5831_19683)"
                                        />
                                        <path
                                          fill-rule="evenodd"
                                          clip-rule="evenodd"
                                    ...
                                    >
                                      <g>
                                        <circle
                                          ...
                                          cy="16"
                                          r="14"
                                          fill="url(#paint0_linear_5831_19691)"
                                        />
                                        <path
                                  ...
                                  <div className="flex gap-4 items-center">
                                    ...
                                    <svg
                                      ...
                                      xmlns="http://www.w3.org/2000/svg"
                                    >
                                      <g clip-path="url(#clip0_5831_19697)">
                                        <path
                              ...
                              <div className="md:w-6/12">
                                ...
                                <div className="flex  gap-2 mt-4">
                                  <Switch
                                    ...
                                    onChange={(val) => {
                                      setFieldValue("explicit", val);
                                    }}
                                    className={`${
                                      values.explicit
                                        ? "bg-[#21A79C]"
                                        : "bg-[#F2F4F7]"
                                    } relative inline-flex h-[18.5px] w-[32px] shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`}
                                  >
                                    ...
                                    <span
                                      ...
                                      className={`${
                                        values.explicit
                                          ? "translate-x-[0.82rem]"
                                          : "translate-x-0"
                                      } pointer-events-none inline-block h-[16px] w-[16px] transform rounded-full bg-white border-slate-600 border shadow-lg ring-0 transition duration-200 ease-in-out`}
                                    />
                                  </Switch>
                                  <div className="text-sm font-medium">
                                    Enable this option if your recording
                                    contains explicit content. This <br />{" "}
                                    information is also exported to your RSS /
                                    iTunes feeds.
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex gap-5">
                              <div className="md:w-6/12">
                                ...
                                <div className="flex  gap-2 mt-4">
                                  ...
                                  <div className="text-sm font-medium">
                                    Enable this option if you want this episode
                                    automatically published on a specific date
                                    and time.
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex gap-5">
                                <div className="md:w-6/12">
                                  ...
                                  <Datetime
                                    ...
                                    onChange={(m) => {
                                      setFieldValue("publish_later_at", m);
                                    }}
                                  />
                                </div>
                              </div>
                          ...
                          <div className="text-right space-x-4">
                            <Link href="/podcast/create-podcast">
                              <Button
                                type="button"
                                className="!text-sm  !from-transparent !to-transparent !text-white border font-semibold"
                              >
                                Update episode
                              </Button>
                            </Link>
                            <Button
                              type="submit"
                              className="!text-sm !from-white !to-white !text-[#063150] font-semibold"
                            >
                              {isSubmitting ? (
                                <svg
                                  className="w-5 h-5"
                            ...
                            {episode.status.toLowerCase() == "unpublished" ? (
          ...
          <Tab.Panel>Content 2</Tab.Panel>
          <Tab.Panel>
            {episode && <ReplaceAudioTab episode={episode} />}
          </Tab.Panel>
        </Tab.Panels>
      </Tab.Group>
    </div>
