...
import { APICall, formatToCurrency } from "@/src/Utils";
...
import { getTransactions } from "@/src/app/api/publishers";
import { TransactionModel } from "@/src/Models/transaction";

const TransactionsTable: React.FC<{ showFilter?: boolean }> = ({
  showFilter,
}) => {
  ...
  const [dateRange, setDateRange] = useState<
    { startDate: string | Date; endDate: string | Date; key: string }[]
  >([
    {
      startDate: "",
      endDate: "",
  ...
  const handleGetTransactions = async (page?: number) => {
    try {
      switch (selectedRange) {
        ...
        case "custom": {
          if (
            moment(dateRange[0].startDate).isValid() &&
            moment(dateRange[0].endDate).isValid()
          ) {
            const response = await APICall(
              getTransactions,
              [
  ...
  useEffect(() => {
    if (dateRange[0].startDate && dateRange[0].endDate) handleGetTransactions();
  }, [dateRange]);
          ...
          <div className="flex">
            <div className="rounded-lg border border-[#D0D5DD] flex">
              <div
                ...
                className={`rounded-l-lg border-r border-[#D0D5DD] text-sm font-inter font-semibold py-[10px] px-4 hover:text-[#101828] hover:bg-white cursor-pointer ${
                  selectedRange == "12" ? "bg[#101828]" : "bg-[#26272B]"
                }`}
              >
                12 months
              </div>
              <div
                ...
                className={`border-r border-[#D0D5DD] text-sm font-inter font-semibold py-[10px] px-4 hover:text-[#101828] hover:bg-white cursor-pointer ${
                  selectedRange == "30" ? "bg[#101828]" : "bg-[#26272B]"
                }`}
              >
                30 days
              </div>
              <div
                ...
                className={`border-r border-[#D0D5DD] text-sm font-inter font-semibold py-[10px] px-4 hover:text-[#101828] hover:bg-white cursor-pointer ${
                  selectedRange == "7" ? "bg[#101828]" : "bg-[#26272B]"
                }`}
              >
                7 days
              </div>
              <div>
                ...
                >
                  <Popover.Button
                    className={`relative outline-none  rounded-r-lg border-r border-[#D0D5DD] text-sm font-inter font-semibold py-[10px] px-4 hover:text-[#101828] hover:bg-white cursor-pointer ${
                      selectedRange == "custom" ? "bg[#101828]" : "bg-[#26272B]"
                    }`}
            ...
            <table className="border-collapse table-auto w-full whitespace-nowrap">
              ...
              <tbody className="bg-[#141414]">
                {transactions?.map((transaction) => {
                  return (
                    <tr key={transaction.reference_number}>
                      ...
                      <td className="py-4 border-b pl-6 text-xs border-t border-[#667085] ">
                        <div className="">
                          <span
                            className={`inline-flex gap-1 text-sm font-medium rounded-full py-1 px-3 border-2 ${
                      ...
                      <td className="py-4 border-b pl-6 text-xs font-medium border-t border-[#667085] ">
                        <div className="">#{transaction.reference_number}</div>
                      </td>
                      <td className="py-4 border-b pl-6 text-xs border-t border-[#667085] ">
                        <div className="text-sm font-bold ">
                          <span className="">₦</span>
                          <span className="ml-1">
                            {formatToCurrency(transaction.amount / 100)}
                          </span>
                        </div>
                      </td>
                      <td className="py-4 border-b pl-6 text-xs border-t border-[#667085] ">
                          ...
                          <Listbox.Options className="absolute mt-4 w-[205px] overflow-auto bg-[#141414] rounded-lg text-sm font-medium right-4 drop-shadow-[0px_3px_5px_rgba(255,255,255,0.1)] z-20">
                            <div className="p-2">
                              <Listbox.Option value={"1"}>
                                {({ active }) => (
                                  <div
                                    className={`py-[0.63rem] px-2 rounded-lg bg-[#1D2939] ${
                                      active ? "bg-[#1D2939]" : ""
                                    }`}
                                  >
                                    Download
